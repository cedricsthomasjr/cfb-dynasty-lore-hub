...
import { onboardingNextStep, DASHBOARD } from "@/lib/dynasty/onboarding";
import { weekLabel, type WeekTypeName } from "@/lib/dynasty/schedule-weeks";

export const metadata = { title: "League Home" };
...
export default async function LeagueHomePage() {
  ...
  // No active dynasty → back to the home menu to create/join/select one.
  if (!active) redirect("/");
  // Onboarding isn't finished (no team, or no schedule) → finish it first.
  const next = onboardingNextStep({
    controlledTeamId: active.controlledTeam?.id ?? null,
    currentWeekId: active.dynasty.currentWeekId,
  });
  if (next !== DASHBOARD) redirect(next);

  const { dynasty, controlledTeam, currentWeek } = active;
  const currentWeekLabel = currentWeek
    ? weekLabel(currentWeek.number, currentWeek.type as WeekTypeName)
    : null;
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <PageHeader
        ...
        description={
          controlledTeam
            ? `League Home — you control ${controlledTeam.name}.`
            : "League Home — your dynasty's front page, powered entirely by verified data."
        }
        action={
